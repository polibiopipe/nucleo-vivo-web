...
assert.equal(cases.filter(item => item.video).length, 3, 'Deben existir tres videos disponibles');
assert.equal(cases.filter(item => !item.video).length, 5, 'Deben existir cinco espacios planificados');
...
console.log('8 casos: 3 videos disponibles y 5 fichas planificadas.');
